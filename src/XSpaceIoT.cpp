/*
  XSpaceIoT.cpp - A simple client for MQTT.
*/
...
XSPSClient::XSPSClient(Client& client) {
    this->_state = MQTT_DISCONNECTED;
    setClient(client);
    this->stream = NULL;
    this->bufferSize = 0;
...
XSPSClient::XSPSClient(IPAddress addr, uint16_t port, Client& client) {
    this->_state = MQTT_DISCONNECTED;
    setServer(addr, port);
    setClient(client);
    this->stream = NULL;
...
XSPSClient::XSPSClient(IPAddress addr, uint16_t port, Client& client, Stream& stream) {
    this->_state = MQTT_DISCONNECTED;
    setServer(addr,port);
    setClient(client);
    setStream(stream);
...
XSPSClient::XSPSClient(IPAddress addr, uint16_t port, MQTT_CALLBACK_SIGNATURE, Client& client) {
    this->_state = MQTT_DISCONNECTED;
    setServer(addr, port);
...
XSPSClient::XSPSClient(IPAddress addr, uint16_t port, MQTT_CALLBACK_SIGNATURE, Client& client, Stream& stream) {
    this->_state = MQTT_DISCONNECTED;
    setServer(addr,port);
    setCallback(callback);
    setClient(client);
...
XSPSClient::XSPSClient(uint8_t *ip, uint16_t port, Client& client) {
    this->_state = MQTT_DISCONNECTED;
    setServer(ip, port);
    setClient(client);
    this->stream = NULL;
...
XSPSClient::XSPSClient(uint8_t *ip, uint16_t port, Client& client, Stream& stream) {
    this->_state = MQTT_DISCONNECTED;
    setServer(ip,port);
    setClient(client);
    setStream(stream);
...
XSPSClient::XSPSClient(uint8_t *ip, uint16_t port, MQTT_CALLBACK_SIGNATURE, Client& client) {
    this->_state = MQTT_DISCONNECTED;
    setServer(ip, port);
    setCallback(callback);
    setClient(client);
...
XSPSClient::XSPSClient(uint8_t *ip, uint16_t port, MQTT_CALLBACK_SIGNATURE, Client& client, Stream& stream) {
    this->_state = MQTT_DISCONNECTED;
    setServer(ip,port);
    setCallback(callback);
    setClient(client);
...
XSPSClient::XSPSClient(const char* domain, uint16_t port, Client& client) {
    this->_state = MQTT_DISCONNECTED;
...
XSPSClient::XSPSClient(const char* domain, uint16_t port, Client& client, Stream& stream) {
    this->_state = MQTT_DISCONNECTED;
    setServer(domain,port);
    setClient(client);
    setStream(stream);
...
XSPSClient::XSPSClient(const char* domain, uint16_t port, MQTT_CALLBACK_SIGNATURE, Client& client) {
    this->_state = MQTT_DISCONNECTED;
    setServer(domain,port);
    setCallback(callback);
    setClient(client);
...
XSPSClient::XSPSClient(const char* domain, uint16_t port, MQTT_CALLBACK_SIGNATURE, Client& client, Stream& stream) {
    this->_state = MQTT_DISCONNECTED;
    setServer(domain,port);
    setCallback(callback);
    setClient(client);
...
boolean XSPSClient::readByte(uint8_t * result) {
...
boolean XSPSClient::readByte(uint8_t * result, uint16_t * index){
...
uint32_t XSPSClient::readPacket(uint8_t* lengthLength) {
    uint16_t len = 0;
...
boolean XSPSClient::beginPublish(const char* topic, unsigned int plength, boolean retained) {
    if (connected()) {
...
int XSPSClient::endPublish() {
 return 1;
}

size_t XSPSClient::write(uint8_t data) {
    lastOutActivity = millis();
...
size_t XSPSClient::buildHeader(uint8_t header, uint8_t* buf, uint16_t length) {
    uint8_t lenBuf[4];
    uint8_t llen = 0;
    uint8_t digit;
    uint8_t pos = 0;
...
boolean XSPSClient::write(uint8_t header, uint8_t* buf, uint16_t length) {
    uint16_t rc;
...
boolean XSPSClient::unsubscribe(const char* topic) {
	size_t topicLength = strnlen(topic, this->bufferSize);
    if (topic == 0) {
        return false;
    }
...
uint16_t XSPSClient::writeString(const char* string, uint8_t* buf, uint16_t pos) {
...
XSPSClient& XSPSClient::setServer(uint8_t * ip, uint16_t port) {
...
XSPSClient& XSPSClient::setCallback(MQTT_CALLBACK_SIGNATURE) {
    this->callback = callback;
    return *this;
}

XSPSClient& XSPSClient::setClient(Client& client){
    this->_client = &client;
    return *this;
}

XSPSClient& XSPSClient::setStream(Stream& stream){
    this->stream = &stream;
    return *this;
}

int XSPSClient::state() {
    return this->_state;
}

boolean XSPSClient::setBufferSize(uint16_t size) {
    if (size == 0) {
...
uint16_t XSPSClient::getBufferSize() {
    return this->bufferSize;
}
XSPSClient& XSPSClient::setKeepAlive(uint16_t keepAlive) {
    this->keepAlive = keepAlive;
    return *this;
}
XSPSClient& XSPSClient::setSocketTimeout(uint16_t timeout) {
